...
@pytest.fixture
def mock_wikipedia_random_page(mocker: Mock) -> Mock:
...
def test_main_uses_en_wikepdia_org(runner: CliRunner, mock_requests_get: Mock) -> None:
...
# From https://medium.com/opsops/how-to-test-if-name-main-1928367290cb
def test_init() -> None:
    """It asserts that the main function is called correctly if it is executed manually."""
    with mock.patch.object(console, "main", return_value=42):
        with mock.patch.object(console, "__name__", "__main__"):
            with mock.patch.object(console.sys, "exit") as mock_exit:
                console.init()
